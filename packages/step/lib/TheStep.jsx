...
class TheStep extends React.Component {
  static Content ({
    ...
    return (
      <div className={c(`the-step-content`, className)}>
        <TheSpin className={`the-step-spin`}
                 cover
                 enabled={spinning}
        />
        {children}
    ...
    return (
      ...
      >
        <TheSpin className={`the-step-spin`}
                 ...
                 enabled={spinning}
        />
        <div className='the-step-scroll'
             style={{ height: this.state.contentHeight }}
        >
          <div className='the-step-inner'
               style={{
                 left: `${100 * step * -1}%`,
                 width: `${100 * stepCount}%`,
               }}
        ...
        <TheCondition unless={hideActions}>
          <TheButtonGroup className='the-step-action'>
            <TheButton className={c('the-step-button', 'the-step-back-button', {
              'the-step-button-hidden': !hasBack,
            })}
                       icon={TheStep.BACK_ICON}
                       onClick={this.onBack}
            >{backText}</TheButton>
            <TheButton className={c('the-step-button', {
              ...
              'the-step-submit-button': isSubmit,
                       ...
                       wide
            >{isSubmit ? submitText : nextText}</TheButton>
          </TheButtonGroup>
        </TheCondition>
      </div>
    )
  ...
  resize (step) {
    ...
    const needsUpdateState = contentHeight && (contentHeight !== this.state.contentHeight)
    if (needsUpdateState) {
